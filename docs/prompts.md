...
| **ĐC** | Chấm công đa ca + cảnh báo trần giờ | ✅ Hoàn chỉnh (docs/10). Khoá (maNV,ngày,maCa); chọn ca khi chấm vào; ca đêm vắt nửa đêm chấm ra theo bản ghi đang mở; cảnh báo tổng >12h/ngày + nghỉ tuần <24h. Lưu giờ VN (+07:00), sort theo instant. |
| **GPS** | Xác minh vị trí + địa bàn làm việc | ✅ Hoàn chỉnh (docs/10). Reverse geocode (Nominatim) hiện đường/phường; chặn chấm nếu ngoài địa bàn (Q1,3,5,8,10,11,Phú Nhuận,Bình Thạnh,Bến Cát trừ Văn Thố,Bàu Bàng); fail-open khi lỗi. Cấu hình `dia_ban_cho_phep`. |
